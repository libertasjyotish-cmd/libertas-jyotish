...
const DETAILED_HOROSCOPE_SPEC = JSON.stringify(
  '本日のアスペクトによる日次＆週次詳細運勢レポート（600文字以上）。'
  + '「【いま、あなたに起きていること】」「【なぜ今そう感じるのか】」「【あなたの選択は】」「【この先に待っているもの】」「【今日・今週の一手】」「【注意日と追い風日】」の6つの見出しで構成し、'
  + 'それぞれの見出しの直前に改行（\n）を入れ、見出しの後も改行してから本文を書くこと。'
  + '【この先に待っているもの】は救いの段落。いまの負荷や整理が「何のための準備か」を、天体の移動時期（トランジットの月・水星・太陽が次の星座やハウスへ移る日、現在のダシャー期の後半に開く領域など）を根拠に、いつ頃からどう好転しやすいかまで具体的に書く（例:「今の抱え込みは、来週◯日に月が第7ハウスへ移った後、対等な協力関係へ切り替えるための整理です。独立心が形になる嬉しい前触れと受け取ってください」）。'
  + '【今日・今週の一手】は行動を3つ、各行の先頭に「・」を付け、曜日または日付・相手・場面まで具体的に書き、最後に負荷を和らげるレメディ（食・色・習慣など日常でできるもの）を1つ添える。'
  + '【注意日と追い風日】は今週の日付を挙げて、それぞれ何を避け何を進めるかを1文で書く。追い風日は注意日と同数以上にし、注意日で締めない。'
  + '最後に、希望を確認する一文（この時期を越えた先の明るい見立て）と、読者への問いかけを1文添えて終える。'
);

// 「状況の言い当て → 理由 → 肯定 → 行動」の型。抽象語で終わらせず、読者が自分の今と照らし合わせられる文にする。
const READING_STYLE = `
  【文章の型（無料・有料とも厳守）】
  - 星の説明から始めず、読者の「いまの状況・感情」から始める。ハウス・ダシャー・トランジットが示す生活領域（仕事／人間関係／家庭／お金／健康習慣／学び）から今週の悩みテーマを1つ選び、「〜な場面が続いていませんか」「〜と感じやすい時期です」のように、読者が自分の現状と照らし合わせられる具体的な描写で言い当てる。
  - 次に、それが起きる理由を天体の配置で1〜2文だけ添える（例:「第6ハウスに入った月・水星が、出生の10室の月を刺激しているためです」）。理由は納得のために添えるもので、主役にしない。
  - 読者が最近下した判断・我慢・迷いに触れ、それを肯定するか、別の角度から意味づけする（「距離を置いた判断は流れに沿っています」など）。
  - 最後は必ず行動で終える。「いつ（曜日・時間帯）」「誰と／どの場面で」「何をする」まで具体的に書く。
  - 【救いを必ず混ぜる】言い当てが的確なほど読者は苦しくなる。負荷・注意点を1つ書いたら、必ず同じ段落か直後に「それが何のための準備か」「いつ頃どう好転しやすいか」を、天体の移動時期やダシャー期の性質を根拠にして添える（例:「今の整理は、月が第7ハウスへ移る来週以降、対等な協力関係へ切り替わるための下準備で、独立の時期が近づいている嬉しい報せです」）。根拠のない気休めは書かないが、注意点だけで終わる段落・文章は禁止。全体の印象は「大変さを理解された上で、希望が見えた」で終える。
  - 「調和」「バランス」「整える」「充実」「エネルギー」「流れに乗る」などの抽象語は最小限にとどめ、使う場合は必ず具体例を続ける。詩的な比喩や決まり文句で1文を終わらせない。
  ...
  if (isPaid && section === 'premium') {
    ...
    以下のJSONスキーマに従って ${outputLanguage} で100%出力してください：
    {
      ...
      "free_reading": {
        ...
        "premium_teaser": "有料の詳細レポートで扱う内容の予告（80〜110文字・1〜2文）。今週の注意日と追い風日が『存在すること』、そして『この状況を動かす具体的な3つの一手』があることを、日付や中身は伏せたまま伝える（例:『今週は◯◯の場面で無理が出やすい日が1日、逆に話を通しやすい日が2日あります。その日付と、上司との切り出し方は詳細レポートで』のように、テーマだけ具体に触れて日付・手順は書かない）。"
      }
    }`;
  }

  // 無料鑑定は「言い当て → 理由 → 今日の一手」まで見せ、日付・手順・週の全体像は有料側に残して premium_teaser で予告する。
  const freeGuideline = isPaid ? `
  【有料会員向け】
  - free_reading（本日の運勢・影響・周期）は要点を短く、premium_reading.detailed_horoscope で同じ悩みテーマを深掘りする。両者で選ぶ悩みテーマは一致させること。
  - detailed_horoscope の【いま、あなたに起きていること】は、free_reading の horoscope より一段具体的に（場面・相手・感情の言葉まで）書く。
  - detailed_horoscope の【この先に待っているもの】は必ず書き、注意点の総量より希望・好転の記述を多くする。
  ` : `
  【無料鑑定の構成】
  - horoscope は「言い当て → 今日の心の状態 → 今日ひとつの具体行動」。月星座 ${moonSign} とナクシャトラ ${nakshatra} の性質を、性格語ではなく「消耗しやすい場面／満たされる条件」として書く。
  - 無料版では「今週の注意日・追い風日の日付」「行動の手順（誰に・いつ・どう切り出すか）」「週全体の流れ」は書かない。それらは premium_teaser で『ある』ことだけを予告する。
  - premium_teaser は売り文句ではなく鑑定文の続きとして自然に書く。「詳細レポートで」「プレミアム鑑定で」のいずれかの語で締める。
  `;
