...
const ELEMENT_EN = { 火: 'fire', 地: 'earth', 風: 'air', 水: 'water' };
...
// 語彙表を持つのは ja / en のみ。es/pt/ar/id は当面 en の語彙で表示する。
function isJa(lang) {
...
function createTerms(rawLang) {
  ...
  const ja = isJa(lang);
  const list = (values, map) => (Array.isArray(values) ? values.map((v) => pick(map, v)) : values);

  return {
    lang,
    outputLanguage: OUTPUT_LANGUAGE[lang],
    sign: (v) => (ja || !v ? v : pick(SIGN_EN, v)),
    nakshatra: (v) => (ja || !v ? v : pick(NAKSHATRA_EN, v)),
    planet: (key, japanese) => (ja ? japanese : pick(PLANET_EN, key, japanese)),
    dignity: (key, japanese) => (ja ? japanese : pick(DIGNITY_EN, key, japanese)),
    planetDomain: (key, japanese) => (ja ? japanese : PLANET_DOMAIN_EN[key] || japanese),
    stone: (v) => (ja || !v ? v : pick(STONE_EN, v)),
    stones: (v) => (ja ? v : list(v, STONE_EN)),
    metal: (v) => (ja || !v ? v : pick(METAL_EN, v)),
    color: (v) => (ja || !v ? v : pick(COLOR_EN, v)),
    direction: (v) => (ja || !v ? v : pick(DIRECTION_EN, v)),
    environment: (v) => (ja ? v : list(v, ENVIRONMENT_EN)),
    workStyleType: (v) => (ja || !v ? v : pick(WORK_STYLE_EN, v)),
    workStyleDetail: (key, japanese) => (ja ? japanese : pick(WORK_DETAIL_EN, key, japanese)),
    element: (v) => (ja || !v ? v : pick(ELEMENT_EN, v)),
    rhythm: (element, japanese) => (ja ? japanese : RHYTHM_EN[element] || japanese),
    houseDomain: (house, japanese) => (ja ? japanese : HOUSE_DOMAIN_EN.find((h) => h.house === house) || japanese),
    dashaSeason: (key, japanese) => (ja ? japanese : pick(DASHA_SEASON_EN, key, japanese)),
    yogaGroup: (rawName, japanese) => (ja ? japanese : pick(YOGA_GROUP_EN, String(rawName || '').toLowerCase(), rawName || japanese)),
    yogaName: (nameEn, japanese) => (ja ? japanese : (nameEn || japanese)),
    sadeSatiPhase: (rawPhase, japanese) => (ja ? japanese : pick(SADE_SATI_PHASE_EN, String(rawPhase || '').toLowerCase(), japanese)),
    remedy: (japanese) => (ja ? japanese : REMEDY_EN),
    chapterTitle: (id, japanese) => (ja ? japanese : pick(CHAPTER_TITLE_EN, id, japanese)),
    reason: ja ? REASON_JA : REASON_EN
  };
}
